...
def _reduce(image, geometry, scale=100):
    import ee
    g = _to_ee(geometry)
    eval_scale = min(scale, 30)
    stats = image.reduceRegion(
        reducer=ee.Reducer.mean().combine(ee.Reducer.median(), sharedInputs=True),
        geometry=g, scale=eval_scale, maxPixels=1e8, bestEffort=True).getInfo()
    v = None
...
def _img_viirs(c):
    import ee; y=c.ndvi_year
    col = ee.ImageCollection("NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG").filterDate(f"{y}-01-01",f"{y}-12-31").select("avg_rad")
    col = ee.ImageCollection(ee.Algorithms.If(col.size().gt(0), col, ee.ImageCollection("NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG").sort("system:time_start",False).limit(12).select("avg_rad")))
    return col.mean().rename("light_pollution")
...
def _img_lst_day(c):
    ...
    col = ee.ImageCollection("MODIS/061/MOD11A1").filterDate(f"{y}-01-01",f"{y}-12-31").select("LST_Day_1km")
    col = ee.ImageCollection(ee.Algorithms.If(col.size().gt(0), col, ee.ImageCollection("MODIS/061/MOD11A1").sort("system:time_start",False).limit(365).select("LST_Day_1km")))
    return col.mean().multiply(0.02).subtract(273.15).rename("LST_Day")

def _img_lst_night(c):
    import ee; y=c.lst_year
    col = ee.ImageCollection("MODIS/061/MOD11A1").filterDate(f"{y}-01-01",f"{y}-12-31").select("LST_Night_1km")
    col = ee.ImageCollection(ee.Algorithms.If(col.size().gt(0), col, ee.ImageCollection("MODIS/061/MOD11A1").sort("system:time_start",False).limit(365).select("LST_Night_1km")))
    return col.mean().multiply(0.02).subtract(273.15).rename("LST_Night")
